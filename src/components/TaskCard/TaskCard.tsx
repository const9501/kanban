...
import {IComment, IFile, ISubTask, TaskActionTypes} from "../../store/reducers/taskReducer";
  ...
  const commentsTree = (id: string) => {
    ...
    if (comments) {
      ...
      comments.forEach(comment => {
        const obj: IRenderComments = {id: comment.id, text: comment.text, children: []}
      ...
      <div className={styles.card}>
        ...
        <div className={styles.informationSection}>
          ...
          <div className={styles.header}>
            <div>
              <EditIcon onClick={() => setDisabled(!disabled)} title={disabled ? 'Редактировать' : 'Сохранить'}
            ...
            <div>
              ...
              <span
                title={new Date(task.creationDate).toLocaleString()}>Создана: {new Date(task.creationDate).toLocaleDateString()}</span>
              {
                task.workingTime ?
              ...
              }
              {
                task.endDate ?
                  <span
                    title={new Date(task.endDate).toLocaleString()}>Завершена: {new Date(task.endDate).toLocaleDateString()}</span> :
                  null
              }
            </div>
          </div>
          ...
          <div className={styles.container}>
            ...
            <div className={styles.valuesContainer}>
              <span>
                {
                  task.status === 'queue' ? 'Очередь в работу' :
                    task.status === 'development' ? 'В работе' :
                      'Готово'
                }
              </span>
              <span>
          ...
          <div className={styles.descriptionWrapper}>
            <div className={styles.descriptionTitle}>
              <DocumentIcon/>
              <span>
              Описание {!task.description && 'пустое'}
            </span>
            </div>
            <Textarea
              ...
              <button
                ...
                onClick={event => {
                  ...
                  if (subTask.length) {
                    setTask({
                      ...task,
                      subTasks: [...task.subTasks, {id: crypto.randomUUID(), task: subTask, completed: false}]
                    })
                    setSubTask('')
                  }
                }}
          ...
          {
            <FilePicker disabled={disabled} selectedFiles={task.files} setSelectedFiles={setSelectedFiles}/>
          }
          ...
          <div className={styles.commentList}>
            {
              renderComments.map((comment) => {
                  return (
                    <Comment key={comment.id} comment={comment} addReplyComment={addReplyComment}/>
                  )
                }
              )
            }
