...
import {ISubTask, ITask, TaskActionTypes} from "../../store/reducers/taskReducer";
...
const TaskForm = ({setOpen}: { setOpen: (open: boolean) => void }) => {
  ...
  const {id} = useParams()
  const tasksQuantity = useAppSelector(state => state.tasks).length
  ...
  const removeSubTask = (id: string) => {
    setSubTasks(
      subTasks.filter(item => item.id !== id)
    )
  }
        ...
        <div className={styles.subTaskInputWrapper}>
          ...
          <button
            ...
            onClick={event => {
              event.preventDefault()
              if (subTask.length) {
                setSubTasks([...subTasks, {id: crypto.randomUUID(), task: subTask, completed: false}])
                setSubTask('')
              }
            }}
          >
